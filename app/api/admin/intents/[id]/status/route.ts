...
import { updateIntentStatusForAdmin } from "@/services/project-service";
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const ret = await updateIntentStatusForAdmin({
      intentId: id,
    ...
    const message = error instanceof Error ? error.message : "管理端意向单状态更新失败";
